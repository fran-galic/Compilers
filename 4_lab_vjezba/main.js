  ...
  for (var key in simulator.CPU._r) {
    if (key !== "sr") {
      ...
      for (var flag in simulator.CPU._f) {
        retVal +=
          flag + ": " + simulator.CPU._getFlag(simulator.CPU._f[flag]) + " ";
      }
      retVal += ") ";
    }
  }
...
function runProgram(frisc_asmsource) {
  ...
  simulator.CPU.onBeforeExecute = function (instruction) {
    debug(
      "## Executing FRISC instruction: " + instructionToString(instruction)
    );
  };
  ...
  simulator.CPU.onStop = function () {
    ...
    console.error("*********************************************************");
    console.error(
      "FRISC processor stopped! Status of CPU R6: " + simulator.CPU._r.r6
    );
    console.error("*********************************************************");
    console.error("");
