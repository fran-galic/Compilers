...
function convertBinaryToInt(value, signed) {
  ...
  for (var i = 0, numberOfBits = value.length; i < numberOfBits - signed; i++) {
    ...
    if (bit !== "0" && bit !== "1") {
      throw (
        "invalid bit in binary string 'value' at position " +
        (numberOfBits - 1 - i) +
        " (" +
        bit +
        ")"
  ...
  return signed && value[0] === "1"
    ? (Math.pow(2, value.length - 1) - retVal) * -1
    : retVal;
}
  ...
  var CPU = {
    ...
    _testCond: function (cond) {
      ...
      if (cond === "") {
        ...
        // ******** ULE         C=0 ili Z=1
        result = !this._getFlag(this._f.C) || !!this._getFlag(this._f.Z);
      } else if (cond === "_UGT") {
        // ******** UGT         C=1 i Z=0
        result = !!this._getFlag(this._f.C) && !this._getFlag(this._f.Z);
      } else if (cond === "_SLT") {
        // ******** SLT         (N xor V)=1
        result = !!this._getFlag(this._f.N) !== !!this._getFlag(this._f.V);
      } else if (cond === "_SLE") {
        // ******** SLE         (N xor V)=1 ili Z=1
        result =
          !!this._getFlag(this._f.N) !== !!this._getFlag(this._f.V) ||
    ...
    _decode: function (statement) {
      ...
      if (op === "MOVE") {
        ...
        if (typeof cond === "undefined") {
          ...
          var isRETI =
            getBitString(statement, 0, 0) === "1" &&
            getBitString(statement, 1, 1) === "0";
          var isRETN =
            getBitString(statement, 0, 0) === "1" &&
            getBitString(statement, 1, 1) === "1";
    ...
    _i: {
      ...
      AND: function (src1, src2, dest) {
        var res =
          this._r[src1] & (typeof src2 === "number" ? src2 : this._r[src2]);
      ...
      ROTL: function (src1, src2, dest) {
        ...
        src2 = src2 % 32;
        var res = convertBinaryToInt(
          src1.substring(src2) + src1.substring(0, src2)
        );
      ...
      ROTR: function (src1, src2, dest) {
        ...
        src2 = src2 % 32;
        var res = convertBinaryToInt(
          src1.substring(32 - src2) + src1.substring(0, 32 - src2)
        );
      ...
      LOAD: function (reg, addr, offset) {
        var destAddr =
          (typeof addr === "string" ? this._r[addr] : 0) +
      ...
      JP: function (cond, dest) {
        if (this._testCond(cond)) {
          this._r.pc =
            ((typeof dest === "string" ? this._r[dest] : dest) & ~0x03) - 4;
        }
      },
      ...
      if (this._r.iif === 0) {
        ...
        if (IO.testInterrupt(3)) {
          ...
          if (this._getFlag(this._f.GIE) === 0) {
            ...
            if (
              (this._getFlag(this._f.INT2) && this._getFlag(this._f.EINT2)) ||
    ...
    run: function (fastest) {
      ...
      if (!fastest) {
        this._runTimer = setInterval(
          this.performCycle.bind(this),
          (1 / this._frequency) * 1000
        );
      } else {
        while (1) {
          try {
    ...
    performCycle: function () {
      ...
      if (decodedInstruction.op !== null && decodedInstruction.args !== null) {
        ...
        this.stop();
        throw new Error(
          "undefined operation code or wrongly defined arguments"
        );
      }
    },
  ...
  var IO = {
    ...
    createFriscCtIoUnit: function (id, options) {
      ...
      if (typeof options.frequency === "undefined") {
        throw new Error(
          "FRISC CT unit must be defined with a frequency parameter."
        );
      }
      ...
      ioUnit.onStateChangeInternal = function (addr, val) {
        if (
          typeof ioUnit.onStateChange !== "undefined" &&
          ioUnit.onStateChange !== null
        ) {
      ...
      ioUnit.determineLocationAndOffset = function (addr) {
        return {
          location: (addr - ioUnit.memMapAddrStart) / 4,
          offset: addr % 4,
        };
      };
    ...
    createIoUnit: function (id, options) {
      if (
        typeof options.memMapAddrCount !== "undefined" &&
        options.memMapAddrCount < 0
      ) {
        throw new Error(
          "Number of memory mapped locations must be non-negative."
        );
      ...
      if (
        typeof options.memMapAddrStart !== "undefined" &&
        options.memMapAddrStart < 0
      ) {
        throw new Error(
          "Memory mapping for io unit is out of addressable memory range."
        );
      } else if (
        typeof options.memMapAddrStart === "undefined" &&
        options.memMapAddrCount === 0
      ...
      if (options.memMapAddrStart % 4 !== 0) {
        throw new Error(
          "Memory mapping for io unit must start from an address that is divisible by 4."
        );
      }

      if (
        typeof options.intLevel !== "undefined" &&
    ...
    addIoUnit: function (ioUnit) {
      ...
      if (mappedUnit !== null) {
        throw new Error(
          "Memory mapping of IO unit overlaps with already connected IO unit:" +
            mappedUnit.id
        );
      }

      mappedUnit = this.testMemoryOverlap(
        ioUnit.memMapAddrStart + ioUnit.memMapAddrCount * 4 - 1
      );
    ...
    testMemoryOverlap: function (memAddr) {
      for (var i = 0; i < this._units.interrupt.length; i++) {
        for (var j = 0; j < this._units.interrupt[i].length; j++) {
          if (
            ...
            memAddr <=
              this._units.interrupt[i][j].memMapAddrStart +
      ...
      for (var i = 0; i < this._units.noninterrupt.length; i++) {
        if (
          ...
          memAddr <=
            this._units.noninterrupt[i].memMapAddrStart +
